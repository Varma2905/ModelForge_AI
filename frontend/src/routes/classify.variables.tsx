...
import { useSelectClassificationFeatures } from "@/hooks/use-training";
...
export const Route = createFileRoute("/classify/variables")({
  beforeLoad: requireAuth,
  component: VariablesPage,
});

// Mirrors backend/app/api/classify_routes.py::_is_valid_classification_target()
// exactly — a column is eligible as a classification target if it's
// categorical, or a numerical column that's actually integer-encoded class
// labels (0/1/2) with few enough distinct values to plausibly be classes
// rather than a continuous regression target.
const MAX_INTEGER_TARGET_CLASSES = 20;

function isEligibleTarget(col: ColumnProfile): boolean {
  if (col.kind === "categorical") return col.unique_count >= 2;
  if (col.kind === "numerical" && col.is_integer_like) {
    return col.unique_count >= 2 && col.unique_count <= MAX_INTEGER_TARGET_CLASSES;
  }
  return false;
}
...
function VariablesPage() {
  ...
  const selectFeatures = useSelectClassificationFeatures();
  ...
  const eligibleTargetCols = allCols.filter(isEligibleTarget);
  ...
  } else if (targetProfile && !isEligibleTarget(targetProfile)) {
    validationMessages.push(
      `Classification requires a categorical target, or a numerical target with a small number of distinct integer values (2-${MAX_INTEGER_TARGET_CLASSES} classes).`,
    );
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      navigate({ to: "/classify/preprocessing" });
    } catch (err) {
      toast.error(
        err instanceof ApiError
          ? err.message
          : "Selected columns are invalid for classification. Please review your selection.",
      );
    }
  };
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-xs text-muted-foreground">
              Classification requires a categorical target, or a numerical column with a small
              number of distinct integer values (2-{MAX_INTEGER_TARGET_CLASSES} classes).
            </p>
          </CardHeader>
          <CardContent>
            {eligibleTargetCols.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                This dataset has no column eligible as a classification target — either a
                categorical column, or a low-cardinality integer-encoded label column.
              </p>
            ) : (
              <RadioGroup
                value={state.target ?? ""}
                ...
                {eligibleTargetCols.map((c) => (
                  ...
                  >
                    ...
                    <Badge variant="outline" className="text-[10px] font-normal">
                      {c.kind === "categorical" ? "Categorical" : `${c.unique_count} classes`}
                    </Badge>
                  </label>
                ))}
              </RadioGroup>
            )}
            {targetProfile && isEligibleTarget(targetProfile) && (
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="rounded-lg border p-3">
                  <div className="text-xs text-muted-foreground mb-1">Classification Type</div>
                  <Badge variant="default">
                    {targetProfile.unique_count === 2 ? "Binary Classification" : "Multiclass Classification"}
                  </Badge>
                </div>
                <div className="rounded-lg border p-3">
                  <div className="text-xs text-muted-foreground mb-1">Number of Classes</div>
                  <span className="text-lg font-semibold">{targetProfile.unique_count}</span>
                </div>
              </div>
            )}
          </CardContent>
...
function FeatureSection({
  ...
  badgeLabel: string;
  cols: ColumnProfile[];
  state: ReturnType<typeof useClassificationAnalysis>["state"];
